...
function draw(gl, n, u_MvpMatrix, mvpMatrix, canvas) {
   ...
   camera.projMatrix.setPerspective(40, canvas.width/canvas.height, 1, 100);
   camera.viewMatrix.setLookAt(camera.eyeX, 1, camera.eyeZ, camera.centerX, 0, 0, 0, 1, 0);
...
// Windmill Blades----------------------------------------------------------// thanks to chuy Flores
   ...
   // windmill.modelMatrix.setIdentity();
   // mvpMatrix.setIdentity();
   // windmill.modelMatrix.translate(1.5, 0.7, -1.8).rotate(angle+90,0,0,1).scale(0.1, 1.6, 0.1);
   // //windmill.modelMatrix.setTranslate(1.5, 0 , -5);
   // mvpMatrix.set(camera.projMatrix).multiply(camera.viewMatrix).multiply(windmill.modelMatrix);
   // setBufferData(gl, Rect.colorBuffer, Rect.colors[0], 3, gl.FLOAT, 'a_Color');
   // gl.uniformMatrix4fv(u_MvpMatrix, false, mvpMatrix.elements);
   ...
   // windmill.modelMatrix.setTranslate(1.5, 0.1, -1.8).scale(0.1, 1.0, 0.1);
   // //windmill.modelMatrix.setTranslate(1.5, 0 , -5);
   // mvpMatrix.set(camera.projMatrix).multiply(camera.viewMatrix).multiply(windmill.modelMatrix);
   // setBufferData(gl, Rect.colorBuffer, Rect.colors[0], 3, gl.FLOAT, 'a_Color');
   // gl.uniformMatrix4fv(u_MvpMatrix, false, mvpMatrix.elements);
   ...
   // windmill.modelMatrix.setTranslate(0.9, 0.7, -1.8).scale(1.0, 0.1, 0.1);
   // // windmill.modelMatrix.setRotate(45, 1, 1, 1);
   // //windmill.modelMatrix.setTranslate(1.5, 0 , -5);
   // mvpMatrix.set(camera.projMatrix).multiply(camera.viewMatrix).multiply(windmill.modelMatrix);
   // setBufferData(gl, Rect.colorBuffer, Rect.colors[1], 3, gl.FLOAT, 'a_Color');
   // gl.uniformMatrix4fv(u_MvpMatrix, false, mvpMatrix.elements);
   // gl.drawElements(gl.TRIANGLES, n, gl.UNSIGNED_BYTE, 0);
   // windmill.modelMatrix.setRotate(0, 1, 1, 1);

   // windmill.modelMatrix.setTranslate(2.0, 0.7, -1.8).scale(1.0, 0.1, 0.1);
   // //windmill.modelMatrix.setTranslate(1.5, 0 , -5);
   // mvpMatrix.set(camera.projMatrix).multiply(camera.viewMatrix).multiply(windmill.modelMatrix);
   // setBufferData(gl, Rect.colorBuffer, Rect.colors[1], 3, gl.FLOAT, 'a_Color');
   // gl.uniformMatrix4fv(u_MvpMatrix, false, mvpMatrix.elements);
